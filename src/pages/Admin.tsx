...
export default function Admin({ db, updateDb, showToast }: any) {
  ...
  const renderSec = () => {
    if(sec === 'dashboard') return (
      <div className="animate-in fade-in duration-300">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="relative overflow-hidden bg-bg shadow-sm">
    ...
    if (sec === 'users') return (
      <div className="animate-in fade-in duration-300">
        ...
        <Card className="bg-bg shadow-sm overflow-hidden border border-border">
              ...
              <tbody className="divide-y divide-border">
                {users.map((u:any) => (
                  <tr key={u.id} className="hover:bg-bg-subtle/50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <Badge variant={u.isAdmin ? 'error' : 'secondary'} className="px-2.5 py-1">
                        {u.isAdmin ? 'Admin' : u.role}
                      </Badge>
                    </td>
                    <td className="py-3 px-4 text-[14px] text-text-tertiary">{u.joined}</td>
                    <td className="py-3 px-4 text-right">
                      <Button variant="ghost" size="sm" onClick={() => adminDelUser(u.id)} className="text-rose hover:bg-rose-light/50 h-8 px-2 group">
                ...
                {posts.map((p:any) => {
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-bg-subtle/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-[14px] text-text-secondary">{p.pinned ? '📌' : '-'}</td>
                      <td className="py-3 px-4 text-right flex justify-end gap-2">
                        <Button variant="ghost" size="sm" onClick={() => togglePin(p.id)} className="h-8 px-2 text-text-secondary hover:text-ink-900">
                          {p.pinned ? <PinOff size={16} /> : <Pin size={16} />}
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => deletePost(p.id)} className="text-rose hover:bg-rose-light/50 h-8 px-2 group">
                          <Trash2 size={16} className="text-rose opacity-70 group-hover:opacity-100" />
                        </Button>
                      </td>
                    </tr>
      ...
      <div className="animate-in fade-in duration-300">
        ...
        <div className="space-y-4">
          {EVENTS_DATA.map((e:any) => (
            <Card key={e.id} className="bg-bg shadow-sm">
              <CardBody className="p-5 flex justify-between items-center flex-wrap gap-4">
                <div>
                  ...
                  <div className="text-[13px] text-text-secondary font-medium">{e.day} {e.mo} · {e.time} · {e.spots} spots left</div>
                </div>
                <Badge variant="success" className="px-3 py-1.5 whitespace-nowrap">Active</Badge>
              </CardBody>
            </Card>
          ))}
        </div>
    ...
    if (sec === 'prompts-a') return (
      <div className="animate-in fade-in duration-300">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {PROMPTS.map((p:any) => (
            <Card key={p.id} className="bg-bg shadow-sm flex flex-col h-full">
              <CardBody className="p-6 flex flex-col h-full">
                ...
                <div className="inline-flex items-center bg-bg-subtle border border-border px-3 py-1.5 rounded-full text-[13px] font-semibold text-text-secondary self-start">
                  💾 {p.saves} saves
                </div>
              </CardBody>
            </Card>
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] bg-bg-subtle w-full max-w-7xl mx-auto px-4 md:px-6 py-8">
