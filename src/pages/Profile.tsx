    ...
    <div className="min-h-screen bg-bg-subtle text-text-primary px-4 md:px-6 py-12 md:py-16">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col items-center text-center mb-16 relative">
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-tr from-accent to-accent-light rounded-full blur-xl opacity-30 group-hover:opacity-50 transition-opacity"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          ...
          <motion.div initial={{y:20, opacity:0}} animate={{y:0, opacity:1}} transition={{delay:0.1}}>
            <Card className="h-full bg-bg shadow-sm">
              <CardBody className="p-8 flex flex-col h-full">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <Badge variant="accent" className="text-[14px]">{progressPct}%</Badge>
                </div>
                
                <div className="mb-4">
                  <ProgressBar progress={progressPct} variant="accent" size="md" showLabel={false} />
                </div>
                ...
                <div className="mt-auto">
                  <Button onClick={() => setPage('lessons')} className="w-full h-12">
                     Resume Learning
                  </Button>
                </div>
              </CardBody>
          ...
          <motion.div initial={{y:20, opacity:0}} animate={{y:0, opacity:1}} transition={{delay:0.2}}>
             <Card className="h-full bg-bg shadow-sm border-border">
              <CardBody className="p-8 flex flex-col h-full">
                ...
                <div className="flex flex-col gap-1 justify-center flex-1 space-y-4">
                  ...
                  <div className="flex justify-between items-center pb-4 border-b border-border/60">
                    <span className="text-text-secondary font-medium flex items-center gap-2"><Star size={16} className="text-text-tertiary"/> PRO Account</span>
        ...
        <div className="mb-8 flex items-center justify-between">
          ...
          {savedLen > 0 && <Button variant="ghost" onClick={() => setPage('prompts')} className="text-accent underline text-[14px]">Browse all</Button>}
        </div>
        
        {savedLen === 0 ? (
          <Card className="bg-bg-subtle border-dashed border-2 p-16 text-center">
            <div className="text-[48px] opacity-20 mb-4 inline-block">💾</div>
            ...
            <Button onClick={() => setPage('prompts')}>Explore Prompts</Button>
          </Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {savedPromptsFull.map((p:any) => (
               <Card key={p.id} className="bg-bg hover:shadow-md transition-shadow group flex flex-col h-full border-border-strong hover:border-text-tertiary">
                 <CardBody className="p-6 flex flex-col flex-1">
                   ...
                   <Button variant="secondary" onClick={() => setPage('prompts')} className="w-full">
                     View in Library
                   </Button>
                 </CardBody>
               </Card>
